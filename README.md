...
given features *x0* and *x1* of the new point, the probability of it being part of the blue group is computed through the hypothesis function *P=1/(1+e−(x0w0+x1w1))*. P ranges from 0 to 1 (because of the sigmoid function) and 1−P is the probability of the new point being a red group one.
...
The logistic regression model is characterized by a set of "weights", corresponding to the coefficients *w0*, *w1*, etc in the hypothesis function (*P=1/(1+e−(x0w0+x1w1))* in the 2D example).

The goal of the training process is to tune the hypothesis function to return a value close to one for examples with label 1, and an output close to 0 for examples with label 0.

Skipping all math and linear algebra related narration (http://cs229.stanford.edu/notes/cs229-notes1.pdf), the training process consist of 3 lines of R code:
